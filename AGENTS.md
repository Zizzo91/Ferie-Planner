...
- `ferie.state(user_id uuid PK → auth.users, profiles jsonb, active_profile text, last_update timestamptz, updated_at)` — **una riga per utente**; le chiavi di `profiles` sono `"Nome-YYYY"` → `{initial, events}`.
...
- All'avvio (dopo login) i dati si caricano dal cloud e popolano il mirror localStorage.
